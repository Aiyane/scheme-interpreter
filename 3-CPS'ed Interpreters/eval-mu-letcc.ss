...
;; 用 scheme 内置的 call/cc 获取一个最终的 k
;; 将原本的 apply-k 的函数名字改成 apply-k*
;; apply-k 变成了 cons，car 是原始的 k，而 cdr 就是 val 了
;; 用无限循环获取 k 与 val 来一步步调用 apply-k*
    ...
    (λ (unparsed-expression)
      (letrec
        ...
        (letcc escape-to-scheme
          (loop (eval-exp (parse-exp unparsed-expression)
                  global-table
                  (make-final-k escape-to-scheme))))))))
